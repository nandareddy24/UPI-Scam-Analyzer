...
def download_android_zip():
    ...
    return Response(
        zip_buffer.getvalue(),
        mimetype="application/zip",
        headers={"Content-Disposition": "attachment; filename=scamshield-android-mobile-app.zip"}
    )
...
def generate_ai_chat_response(user_msg):
    msg_lower = user_msg.lower().strip()

    found_upis = re.findall(r'[\w.-]+@[\w.-]+', user_msg)
    found_urls = re.findall(r'https?://[^\s]+', user_msg)

    reasons = []
    is_scam = False
    scam_type = "Potential Cyber Fraud"

    # 1. Flexible UPI PIN & Collect Scam Intent (Handles typos: reciving, receiv, asking, enter)
    has_pin = "pin" in msg_lower or "p.i.n" in msg_lower or "otp" in msg_lower or "upipin" in msg_lower
    receive_terms = ["reciv", "receiv", "reciev", "refund", "credit", "got", "get", "claim", "money", "asking", "enter", "pay", "send", "give", "transfer"]

    if has_pin and any(t in msg_lower for t in receive_terms):
        is_scam = True
        scam_type = "UPI Collect & PIN Scam"
        reasons.append("Entering your 4 or 6-digit UPI PIN is ONLY required to SEND money. You NEVER enter a PIN to receive money, accept refunds, or check bank balance.")
    elif "reciv" in msg_lower or "receiv" in msg_lower or "refund" in msg_lower:
        if "asking" in msg_lower or "request" in msg_lower or "collect" in msg_lower or "pin" in msg_lower:
            is_scam = True
            scam_type = "UPI Refund / Collect Fraud"
            reasons.append("Scammers send 'Collect Money' requests disguised as refunds. Entering any PIN transfers money FROM your account TO the scammer.")

    # 2. Electricity Bill Disconnection Fraud
    if any(t in msg_lower for t in ["electric", "power", "disconnect", "light bill", "bill unpaid"]):
        is_scam = True
        scam_type = "Fake Electricity Bill Disconnection Fraud"
        reasons.append("Utility boards never send SMS messages demanding urgent bill payments via personal WhatsApp/mobile numbers.")

    # 3. Prize / Lottery Claims
    if any(t in msg_lower for t in ["lottery", "kbc", "winner", "lakh", "crore", "prize"]):
        is_scam = True
        scam_type = "Fake Prize & Lottery Claim Scam"
        reasons.append("Legitimate lottery or contest providers never require advance registration fees or UPI transfers.")

    # 4. Bank Account & KYC Phishing
    if any(t in msg_lower for t in ["kyc", "block", "bank", "suspend", "pan", "account freeze"]):
        is_scam = True
        scam_type = "Bank Account / KYC Phishing Fraud"
        reasons.append("Banks never request panic updates for KYC via unverified SMS links or personal WhatsApp numbers.")

    # 5. Remote Access APK Downloads
    if any(t in msg_lower for t in ["apk", "download", "install", "anydesk", "quicksupport"]):
        is_scam = True
        scam_type = "Malicious APK Remote Access Scam"
        reasons.append("Installing unverified APK files (e.g. AnyDesk, TeamViewer QuickSupport) grants scammers full remote control of your phone.")

    if found_upis:
        bump, matches = check_blacklist_or_community(found_upis[0], "UPI")
        if bump > 0:
            is_scam = True
            reasons.append(f"The handle '{found_upis[0]}' is flagged on the global cyber crime blacklist!")

    if found_urls:
        bump, matches = check_blacklist_or_community(found_urls[0], "URL")
        if bump > 0 or any(k in found_urls[0] for k in ["login", "verify", "bank", "bit.ly"]):
            is_scam = True
            reasons.append(f"The link '{found_urls[0]}' exhibits phishing characteristics or masked domain parameters.")

    if is_scam:
        bullet_list = "\n".join([f"• {r}" for r in reasons])
        return f"🚨 **Verdict: Likely a {scam_type}**\n\nThis message is **NOT genuine**.\n\n**Why it is a scam:**\n{bullet_list}\n\n**What you should do:**\n1. Do **NOT** click any links or scan QR codes.\n2. Do **NOT** enter your UPI PIN.\n3. Report immediately on National Cyber Crime Helpline **1930** or cybercrime.gov.in."
    else:
        return f"🟢 **Verdict: Low Risk / Appears Safe**\n\nI analyzed your query and didn't find immediate scam triggers.\n\n**Golden Rule:** Remember, UPI PIN is **ONLY** required to pay money out of your bank account—never to receive money."
...
def api_v1_chat():
    ...
    if not message:
        return jsonify({"reply": "Please enter a question or message snippet."})
